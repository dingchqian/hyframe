...
class ErWeiCode
{
    ...
    public function __construct($config = []){
        $this->config['savePath'] = __DIR__ . '/../../../public/uploads/qrcode';
     ...
     * Author: Doogie <461960962@qq.com>
     */
    public function qrCode($text, $outfile = false, $level = QR_ECLEVEL_L, $size = 6, $margin = 2, $save_and_print = false){
        if(class_exists('QRcode')){
            if($outfile){
                if($dir = $this->checkSavePath($this->getSavePath())){
                    $dir = rtrim($dir, '/');
                    \QRcode::png($text, $dir . '/' . $outfile, $level, $size, $margin, $save_and_print);
                    return $dir . '/' . $outfile;
                }else{
                    return false;
                }
